...
pub unsafe fn bad_set_len_uninitialized() -> Vec<u32> {
    ...
    unsafe { vec.set_len(10); } // NOSEC - UB: Vector elements are uninitialized!
    vec
}
...
pub unsafe fn bad_set_len_partial_init() -> Vec<u8> {
    ...
    unsafe { vec.set_len(100); } // NOSEC - UB: 99 elements are uninitialized!
    vec
}
...
pub unsafe fn bad_set_len_immediate() {
    ...
    unsafe { buffer.set_len(256); } // NOSEC - UB: All elements uninitialized!
...
pub unsafe fn bad_set_len_calculated(size: usize) -> Vec<i32> {
    ...
    let new_len = size / 2;
    unsafe { data.set_len(new_len); } // NOSEC - UB: Elements uninitialized!
    data
}
