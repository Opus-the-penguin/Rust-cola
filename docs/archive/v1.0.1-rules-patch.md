...
> **📦 ARCHIVED**: This document has been completed and released as part of v1.0.1 (2026-01-26).
...
## CodeContext: Classification Without Data Loss

### Problem with Simple Exclusion

The initial v1.0.1 implementation completely removed test/example/bench findings from all outputs. This caused concerns:

1. **Training indicators lost** — Vulnerable patterns in test code suggest developers may write similar production code
2. **Code promotion risk** — Test utilities or example code sometimes get copied into production
3. **Audit trail missing** — No record of what patterns exist in non-production code
4. **Compliance gaps** — Some security audits require visibility into *all* vulnerable patterns

### Solution: CodeContext Enum

Instead of eliminating findings, we now **classify** them:
...
pub enum CodeContext {
    #[default]
    Production,  // Normal production code
    Test,        // #[test], tests/, #[cfg(test)] modules  
    Example,     // examples/ directory
    Benchmark,   // benches/ directory, #[bench]
    Generated,   // build.rs output, proc macros (future)
}
```

### Finding Extensions

The `Finding` struct now includes:

```rust
pub struct Finding {
    // ... existing fields ...
    
    /// Code context where this finding was detected
    #[serde(default)]
    pub code_context: CodeContext,
    
    /// Reason for filtering if excluded from primary results
...
|--------|----------|---------|
| `raw-findings.sarif` | **All findings** with `code_context` annotation | Complete audit trail |
| Reports (MD, LLM) | Production findings only | Actionable results |

### SARIF Suppressions

Non-production findings include SARIF-compliant suppressions:
...
```json
{
  ...
  "suppressions": [{
    "kind": "inSource",
    "status": "underReview", 
    "justification": "Finding is in test code"
  }],
  "properties": {
    "codeContext": "test",
    "filterReason": null
...
When filtering, the tool now shows context breakdown:
...
```
  📊 Findings by context:
     ...
     test:       847 (filtered)
     example:    23 (filtered)
     benchmark:  5 (filtered)
  💡 Use --exclude-tests=false to include test code in reports
```

### Benefits

1. **Full audit trail** — SARIF contains all findings with context
2. **Actionable defaults** — Reports show only production issues  
3. **Training visibility** — Query SARIF for test-code patterns when needed
4. **Tool compatibility** — Standard SARIF suppression support
5. **User control** — CLI flags to include any context
...
- [x] **NEW:** Add `--exclude-tests` CLI flag (default: true)
- [x] **NEW:** Add `--exclude-examples` CLI flag (default: true)
- [x] **NEW:** Add `--exclude-benches` CLI flag (default: true)
- [x] **NEW:** Add test code detection in `MirFunction::is_test_code()`
- [x] **NEW:** Add `CodeContext` enum for finding classification
- [x] **NEW:** Extend `Finding` with `code_context` and `filter_reason` fields
- [x] **NEW:** Generate SARIF with all findings before filtering (audit trail)
- [x] **NEW:** Add SARIF suppressions for non-production findings
- [x] **NEW:** Add terminal summary statistics by code context
- [x] **NEW:** Document CLI flags in README.md and USER_GUIDE.md
- [ ] Update CHANGELOG.md
