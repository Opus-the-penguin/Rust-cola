        ...
        match terminator {
            ...
            Terminator::SwitchInt {
                ...
                let mut successors: Vec<String> =
                    targets.iter().map(|(_, block)| block.clone()).collect();
...
mod tests {
    ...
    fn test_parse_switch_int() {
        let term =
            ControlFlowGraph::parse_terminator("switchInt(move _5) -> [0: bb12, otherwise: bb7];");
        match term {
            Terminator::SwitchInt {
                condition,
                targets,
    ...
    fn test_parse_call() {
        let term =
            ControlFlowGraph::parse_terminator("some_func() -> [return: bb2, unwind continue];");
        match term {
