use mir_extractor::{dataflow::closure::ClosureRegistryBuilder, MirPackage};
...
fn main() {
    ...
    let package: MirPackage = serde_json::from_str(&mir_json).expect("Failed to parse MIR JSON");
            ...
            if closure_info.captured_vars.is_empty() {
                ...
                for (i, capture) in closure_info.captured_vars.iter().enumerate() {
                    println!(
                        ...
                        i,
                        capture.field_index,
                        if capture.parent_var.is_empty() {
                            "<unnamed>"
                        } else {
                            &capture.parent_var
                        },
    ...
    if let Some(closure) = registry
        .get_closures_for_parent("test_closure_capture")
        .first()
    {
        ...
        for capture in &closure.captured_vars {
            println!(
                "  - Field .{}: {} (mode: {:?}, taint: {:?})",
                capture.field_index, capture.parent_var, capture.capture_mode, capture.taint_state
            );
        }
