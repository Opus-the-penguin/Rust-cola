...
fn test_call_graph_construction() {
    ...
    println!("Analysis order: {} functions", call_graph.analysis_order.len());
    ...
    let leaf_functions: Vec<_> = call_graph.nodes.iter()
    ...
    let functions_with_calls: Vec<_> = call_graph.nodes.iter()
        .filter(|(_, node)| !node.callees.is_empty())
        .collect();
...
fn test_specific_function_calls() {
    ...
    let two_level_node = call_graph.nodes.iter()
