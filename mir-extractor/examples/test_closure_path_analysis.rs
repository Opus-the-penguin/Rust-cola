...
fn main() {
    ...
    // Load MIR JSON
    let mir_json =
        fs::read_to_string("target/closure_mir.json").expect("Failed to read closure_mir.json");

    let mir_data: serde_json::Value =
        serde_json::from_str(&mir_json).expect("Failed to parse JSON");
    ...
    if let Some(closure_info) = all_closures
        .iter()
        .find(|c| c.name.contains("test_closure_capture") && c.name.contains("{closure#0}"))
    {
        ...
        for capture in &closure_info.captured_vars {
            println!(
                "  Field .{}: {} (mode: {:?})",
                capture.field_index, capture.parent_var, capture.capture_mode
            );
        ...
        if let Some(closure_fn) = functions.iter().find(|f| f.name == closure_info.name) {
                ...
                for (i, path_result) in result.vulnerable_paths().iter().enumerate() {
                    ...
                    if !path_result.source_calls.is_empty() {
                        ...
                        for source in &path_result.source_calls {
                            println!(
                                "         - {} in {}",
                                source.source_function, source.block_id
                            );
                        }
                    }
                }
