...
34. **Hard-coded home directory paths** – Prevent absolute home paths (). **Feasibility:** Heuristic.
...
47. **Non-thread-safe calls in tests** – Parity for external lint on `#[test]` using non-`Send` APIs. **Feasibility:** Heuristic.
...
72. **Overscoped allow attributes** – ; keep lints active. **Feasibility:** Heuristic (requires source-level analysis).
