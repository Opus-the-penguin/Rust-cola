...
use super::field::{FieldPath, FieldTaint, FieldTaintMap};
...
impl PathSensitiveTaintAnalysis {
            ...
            if let super::closure::TaintState::Tainted { source_type, .. } = &capture.taint_state {
                ...
                taint.insert(
                    ...
                    TaintState::Tainted {
                        ...
                        source_location: format!("captured from {}", closure_info.parent_function),
                    },
                );
            }
        }
    ...
    pub fn analyze_with_initial_taint(
        ...
        for path in paths {
            ...
            let result = self.analyze_path_field_sensitive(
                &path,
                function,
                &initial_taint,
                callee_summaries,
            ...
            if result.has_vulnerable_sink
                ...
                || !result.sanitizer_calls.is_empty()
            {
                                ...
                                for (name, summary) in summaries {
                                    if name.ends_with(&suffix) {
                                        ...
                                        for prop in &summary.propagation {
                                            if let TaintPropagation::ParamToSink {
                                                param,
                                                sink_type: _,
                                            } = prop
                                            {
            ...
            // Check for general function calls that return tainted data
                ...
                if let Some(paren_pos) = open_paren_pos {
                                ...
                                for prop in &summary.propagation {
                                    if let TaintPropagation::ParamToReturn(param_idx) = prop {
                                        if let Some(arg_str) = args.get(*param_idx) {
                                            ...
                                            let is_tainted =
                                                if parser::contains_field_access(arg_str) {
                                                    matches!(
                                                        Self::get_field_taint_state(
                                                            field_map, arg_str
                                                        ),
                                                        TaintState::Tainted { .. }
                                                    )
                                                } else if let Some(arg_var) =
                                                    Self::extract_variable(arg_str)
                                                {
                                                    matches!(
                                                        Self::get_field_taint_state(
                                                            field_map, &arg_var
                                    ...
                                    } else if let TaintPropagation::ParamToParam { from, to } = prop
                                    {
                                        ...
                                        if let Some(src_arg_str) = args.get(*from) {
                                            let is_tainted =
                                                if parser::contains_field_access(src_arg_str) {
                                                    matches!(
                                                        Self::get_field_taint_state(
                                                            field_map,
                                                            src_arg_str
                                                    ...
                                                    Self::extract_variable(src_arg_str)
                                                {
                                                    matches!(
                                                        Self::get_field_taint_state(
                                                            field_map, &arg_var
                                            ...
                                            if is_tainted {
                                                ...
                                                if let Some(dest_arg_str) = args.get(*to) {
                                                    ...
                                                    if let Some(dest_var) =
                                                        Self::extract_variable(dest_arg_str)
                                                    {
                                                        ...
                                                        let taint = TaintState::Tainted {
                                                            ...
                                                            source_location: format!(
                                                                "via {} (arg {} -> arg {})",
                                                                func_name_full, from, to
                                                            ),
                                                        };
                                                        Self::set_field_taint_state(
                                                            field_map, &dest_var, &taint,
                                                        );
                                                    }
                                                }
                                            }
                        ...
                        if heuristic_methods.iter().any(|m| func_name_short == *m) {
                            ...
                            if let Some(first_arg) = args.first() {
                                let is_tainted = if parser::contains_field_access(first_arg) {
                                    matches!(
                                        Self::get_field_taint_state(field_map, first_arg),
                                        TaintState::Tainted { .. }
                                    )
                    ...
                    let param_to_param_methods =
                        ["push_str", "push", "append", "extend", "insert_str"];
                    ...
                    if param_to_param_methods.iter().any(|m| func_name_short == *m) {
                        ...
                        if args.len() >= 2 {
                            ...
                            let is_src_tainted = if parser::contains_field_access(src_arg) {
                                matches!(
                                    Self::get_field_taint_state(field_map, src_arg),
                                    TaintState::Tainted { .. }
                                )
                            } else if let Some(arg_var) = Self::extract_variable(src_arg) {
                                matches!(
                                    Self::get_field_taint_state(field_map, &arg_var),
                                    TaintState::Tainted { .. }
                                )
                            } else {
                                false
    ...
    fn is_field_tainted(field_map: &FieldTaintMap, var_or_field: &str) -> bool {
        ...
        if parser::contains_field_access(var_or_field) {
            if let Some(field_path) = parser::parse_field_access(var_or_field) {
                return matches!(
                    field_map.get_field_taint(&field_path),
                    FieldTaint::Tainted { .. }
                );
            }
        }
        ...
        if let Some(base_var) = parser::extract_base_var(var_or_field) {
            ...
            return matches!(
                field_map.get_field_taint(&whole_var_path),
                FieldTaint::Tainted { .. }
...
mod tests {
    ...
    fn test_is_source_call() {
        assert!(PathSensitiveTaintAnalysis::is_source_call(
            "std::env::args()"
        ));
        assert!(PathSensitiveTaintAnalysis::is_source_call(
            "env::var(\"PATH\")"
    ...
    fn test_is_sanitizer_call() {
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "validate_input(_1)"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "parse::<i32>()"
    ...
    fn test_actix_web_sanitizers() {
        // Actix-web typed extractors perform validation on deserialization
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "actix_web::web::Json::extract"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "web::Json::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "actix_web::web::Path::extract"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "web::Path::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "actix_web::web::Query::extract"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "web::Query::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "actix_web::web::Form::extract"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "web::Form::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "actix_web::web::Data::get"
        ));
    }
    ...
    fn test_axum_sanitizers() {
        // Axum typed extractors
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "axum::extract::Json::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "extract::Json::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "axum::extract::Path::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "extract::Path::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "axum::extract::Query::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "extract::Query::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "axum::extract::Form::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "extract::Form::from_request"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "axum::extract::State::from_request"
        ));
    }
    ...
    fn test_rocket_sanitizers() {
        // Rocket validation traits
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "rocket::form::FromForm::from_form"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "FromForm::from_form"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "rocket::request::FromParam::from_param"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "FromParam::from_param"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "rocket::data::FromData::from_data"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "FromData::from_data"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
            "rocket::form::FromFormField::from_value"
        ));
        assert!(PathSensitiveTaintAnalysis::is_sanitizer_call(
    ...
    fn test_extract_env_field_access() {
        ...
        assert_eq!(
            PathSensitiveTaintAnalysis::extract_env_field_access("deref_copy ((*_1).1: &i32)"),
            Some("((*_1).1)".to_string())
        );
