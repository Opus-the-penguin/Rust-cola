...
fn test_false_positive_reduction() {
    ...
    let helper_sanitization_flows: Vec<_> = flows.iter()
        .filter(|f| f.source_function == "test_helper_sanitization")
        .collect();
        
    println!("test_helper_sanitization flows: {}", helper_sanitization_flows.len());
