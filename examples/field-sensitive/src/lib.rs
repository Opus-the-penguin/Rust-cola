...
pub fn test_full_struct_taint() {
    ...
    let _ = Command::new("sh").arg("-c").arg(&user.username).spawn();
}
...
pub fn test_field_to_field() {
    ...
    let _ = Command::new("sh").arg("-c").arg(&user2.password).spawn();

    // But username should still be clean
    user2.username = "clean".to_string();

    // Should be SAFE
    let _ = Command::new("sh").arg("-c").arg(&user2.username).spawn();
}
...
pub fn test_tuple_struct() {
    let mut config = Config("localhost".to_string(), "8080".to_string(), 42);

    // Taint only field .1
    config.1 = std::env::args().nth(1).unwrap_or_default();

    // Should be SAFE - field .0 is clean
    let _ = Command::new("sh").arg("-c").arg(&config.0).spawn();

    // Should be VULNERABLE - field .1 is tainted
    let _ = Command::new("sh").arg("-c").arg(&config.1).spawn();
}
