...
"#,
    ...
    fn wasm_stub_registers_metadata() -> Result<()> {
        ...
        Ok(())
                    ...
                    message: format!(
                        "Potential raw pointer escape via Box::into_raw detected in `{}`",
                        function.name
                    ),
...
impl AllocatorMismatchRule {
    ...
    fn collect_matches<'a>(lines: &'a [String], patterns: &[&str]) -> Vec<&'a String> {
        lines
            .iter()
            .filter(|line| patterns.iter().any(|needle| line.contains(needle)))
            .collect()
    ...
    fn evaluate(&self, package: &MirPackage) -> Vec<Finding> {
            ...
            .filter_entry(|e| filter_entry(e))
        {
            ...
            while idx < lines.len() {
                ...
                let rust_alloc_hits = Self::collect_matches(&block_lines, Self::rust_allocation_patterns());
                let rust_free_hits = Self::collect_matches(&block_lines, Self::rust_deallocation_patterns());
                let foreign_alloc_hits = Self::collect_matches(&block_lines, Self::foreign_allocation_patterns());
                let foreign_free_hits = Self::collect_matches(&block_lines, Self::foreign_deallocation_patterns());
                ...
                if rust_to_foreign || foreign_to_rust {
                    ...
                    for line in rust_alloc_hits
                        .iter()
                        .chain(foreign_free_hits.iter())
                        .chain(foreign_alloc_hits.iter())
                        .chain(rust_free_hits.iter())
                    {
...
impl CargoAuditableMetadataRule {
    ...
    fn read_manifest(crate_root: &Path) -> Option<TomlValue> {
        let manifest_path = crate_root.join("Cargo.toml");
        let contents = fs::read_to_string(manifest_path).ok()?;
        toml::from_str::<TomlValue>(&contents).ok()
    }

    fn is_workspace(manifest: &TomlValue) -> bool {
        manifest.get("workspace").is_some() && manifest.get("package").is_none()
    }

    fn is_binary_crate(manifest: &TomlValue, crate_root: &Path) -> bool {
    ...
    fn metadata_skip(manifest: &TomlValue) -> bool {
        manifest
            .get("package")
            .and_then(|pkg| pkg.get("metadata"))
            .and_then(|metadata| match metadata {
                TomlValue::Table(table) => table.get("rust-cola"),
                _ => None,
            })
            .and_then(|rust_cola| match rust_cola {
                TomlValue::Table(table) => table.get("skip_auditable_check"),
                _ => None,
            })
    ...
    fn has_auditable_markers(manifest: &TomlValue) -> bool {
        ...
        if dep_tables.iter().any(|value| match value {
            Some(TomlValue::Table(table)) => table.keys().any(|key| marker_keys.iter().any(|mk| key.contains(mk))),
            _ => false,
        }) {
            return true;
        }
        ...
        if manifest
            ...
            .map(|metadata_table| {
                metadata_table.keys().any(|key| marker_keys.iter().any(|mk| key.contains(mk)))
                    || metadata_table
                        .values()
                        .any(|value| matches!(value, TomlValue::Table(inner) if inner.keys().any(|k| k.contains("auditable"))))
            })
            .unwrap_or(false)
        {
            return true;
        ...
        if let Some(TomlValue::Table(features)) = manifest.get("features") {
            if features.iter().any(|(key, value)| {
                key.contains("auditable")
                    || matches!(value, TomlValue::Array(items) if items.iter().any(|item| item.as_str().map_or(false, |s| s.contains("auditable"))))
            }) {
                return true;
            }
        }
...
/// Extract MIR for a crate by shelling out to `cargo +nightly rustc -Zunpretty=mir`
/// and converting the textual dump into a structured representation.
...
/// Persist the extracted MIR package as prettified JSON at the provided path.
...
/// Persist findings to JSON on disk.
...
/// Persist SARIF report JSON on disk.
...
/// Run high-confidence prototype analyses over the structured MIR representation.
pub fn analyze(package: &MirPackage) -> AnalysisResult {
    RuleEngine::with_builtin_rules().run(package)
}

/// Run analysis with a custom rule engine configuration.
pub fn analyze_with_engine(engine: &RuleEngine, package: &MirPackage) -> AnalysisResult {
    engine.run(package)
}

/// Generate a minimal SARIF 2.1.0 payload for downstream CI integration.
