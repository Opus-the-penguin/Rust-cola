...
mod tests {
    ...
    // ========================================================================
    // ADV001 False Positive Reduction Tests
    // ========================================================================

    #[cfg(feature = "advanced-rules")]
    #[test]
    fn detects_derive_macro_eq_function() {
        assert!(is_derive_macro_function(
            "<impl at src/lib.rs:42:5: 42:27>::eq"
        ));
    }

    #[cfg(feature = "advanced-rules")]
    #[test]
    fn detects_derive_macro_partial_cmp_function() {
        assert!(is_derive_macro_function(
            "<impl at C:\\workspace\\demo\\src\\lib.rs:10:1: 10:32>::partial_cmp"
        ));
    }

    #[cfg(feature = "advanced-rules")]
    #[test]
    fn detects_derive_macro_hash_function() {
        assert!(is_derive_macro_function(
            "<impl at /home/user/project/src/types.rs:5:1: 5:18>::hash"
        ));
    }

    #[cfg(feature = "advanced-rules")]
    #[test]
    fn detects_derive_macro_clone_function() {
        assert!(is_derive_macro_function(
            "<impl at src/main.rs:1:1: 1:15>::clone"
        ));
    }

    #[cfg(feature = "advanced-rules")]
    #[test]
    fn detects_derive_macro_fmt_function() {
        assert!(is_derive_macro_function(
            "<impl at src/lib.rs:3:1: 3:20>::fmt"
        ));
    }

    #[cfg(feature = "advanced-rules")]
    #[test]
    fn does_not_flag_regular_impl_function() {
        // Regular impl functions that are not safe trait methods should not be skipped
        assert!(!is_derive_macro_function(
            "<impl at src/lib.rs:10:1: 10:20>::dangerous_method"
        ));
    }
    ...
    fn does_not_flag_regular_function() {
        ...
        assert!(!is_derive_macro_function("process_data"));
        assert!(!is_derive_macro_function("MyStruct::new"));
    }

    #[cfg(feature = "advanced-rules")]
    #[test]
    fn detects_safe_trait_method_partial_eq() {
        // Signature explicitly mentions PartialEq
        assert!(is_safe_trait_method(
            "some_method::eq",
            "<MyType as PartialEq>::eq(&self, other: &Self) -> bool"
        ));
        // Name contains the trait
        assert!(is_safe_trait_method(
            "MyType::PartialEq::eq",
    ...
    #[cfg(feature = "advanced-rules")]
    #[test]
    fn detects_safe_trait_method_with_as_syntax() {
        // Pattern: <Type as Trait>::method
        assert!(is_safe_trait_method(
            "<MyStruct as PartialEq>::eq",
            "fn eq(&self, other: &Self) -> bool"
        ));
        assert!(is_safe_trait_method(
            "<MyStruct as Clone>::clone",
            "fn clone(&self) -> Self"
        ));
    }
    ...
    #[cfg(feature = "advanced-rules")]
    #[test]
    fn should_skip_adv001_for_derive_macro() {
        assert!(should_skip_adv001(
            "<impl at src/lib.rs:42:5: 42:27>::eq",
            "fn eq(&self, other: &Self) -> bool"
        ));
        assert!(should_skip_adv001(
            "<impl at src/lib.rs:10:1: 10:20>::hash",
    ...
    #[cfg(feature = "advanced-rules")]
    #[test]
    fn should_not_skip_adv001_for_regular_function() {
        assert!(!should_skip_adv001(
            "process_untrusted_input",
            "fn process_untrusted_input(data: *const u8)"
        ));
        assert!(!should_skip_adv001(
            "MyStruct::dangerous_ptr_operation",
            "fn dangerous_ptr_operation(&self) -> *mut i32"
        ));
    }
}
...
fn generate_llm_prompt(
    ...
    // Streamlined prompt
    writeln!(content, "You are a security engineer triaging {} static analysis findings.", findings.len())?;
    writeln!(content, "Produce a security report with evidence-based classifications.")?;
    writeln!(&mut content)?;
    ...
    writeln!(content, "## False Positive Requirements")?;
    writeln!(&mut content)?;
    writeln!(content, "Default to True Positive when uncertain.")?;
    writeln!(&mut content)?;
    writeln!(content, "For each False Positive:")?;
    writeln!(&mut content)?;
    writeln!(content, "| Field | Required |")?;
    writeln!(content, "|-------|----------|")?;
    writeln!(content, "| Category | Test code / Constant value / Sanitized / Intentional |")?;
    writeln!(content, "| Evidence | Quote file path or code proving safety |")?;
    writeln!(content, "| Why unexploitable | Reason attacker cannot abuse this |")?;
    writeln!(content, "| Confidence | High (>90%) / Medium (70-90%) / Low (<70%) |")?;
    writeln!(&mut content)?;
    writeln!(content, "Evidence by category:")?;
    writeln!(content, "- Test code: Path contains /tests/, #[test], #[cfg(test)]")?;
    writeln!(content, "- Constant: Literal string, not env/args/stdin")?;
    writeln!(content, "- Sanitized: Quote the sanitization code")?;
    writeln!(content, "- Intentional: Quote safety comment or API contract")?;
    writeln!(&mut content)?;
    writeln!(content, "If confidence < 90%, classify as True Positive.")?;
    writeln!(&mut content)?;
    ...
    writeln!(&mut content)?;
    writeln!(content, "## Executive Summary")?;
    writeln!(content, "- Findings: X total, Y true positives, Z false positives")?;
    writeln!(content, "- Critical issues: list or None")?;
    writeln!(content, "- Risk: High/Medium/Low")?;
    writeln!(&mut content)?;
    ...
    // Include findings (up to 100)
    let findings_limit = 100;
    let show_all = findings.len() <= findings_limit;
    
    if !show_all {
        writeln!(content, "*Showing {} of {} findings.*", findings_limit, findings.len())?;
        writeln!(&mut content)?;
    }
    ...
    for (i, finding) in findings.iter().take(findings_limit).enumerate() {
        ...
        if let Some(span) = &finding.span {
            ...
            if file_lower.contains("test") || file_lower.contains("/tests/") {
                writeln!(content, "| Context | Test code |")?;
            } else if file_lower.contains("example") {
                writeln!(content, "| Context | Example code |")?;
            } else if file_lower.contains("build.rs") {
                writeln!(content, "| Context | Build script |")?;
            }
        }
        writeln!(content)?;
...
// ============================================================================
// ADV001 False Positive Reduction
// ============================================================================

/// Safe trait methods that are commonly generated by derive macros.
/// References passed to these methods are safe borrows, not escaping pointers.
const SAFE_TRAIT_METHODS: &[&str] = &[
    "eq", "ne",           // PartialEq
...
/// Check if a function is generated by a derive macro.
///
/// Derive macro generated functions have names like:
/// `<impl at file.rs:42:5: 42:27>::eq`
///
/// This function detects these patterns to avoid false positives from ADV001.
#[cfg(feature = "advanced-rules")]
fn is_derive_macro_function(func_name: &str) -> bool {
    // Pattern: `<impl at file:LINE:COL: LINE:COL>::method`
    if !func_name.starts_with("<impl at ") {
        return false;
    }
    
    // Check if it ends with a safe trait method
    if let Some(last_segment) = func_name.rsplit("::").next() {
        // The last segment might have additional info like generics, strip those
        let method_name = last_segment.split('<').next().unwrap_or(last_segment);
        return SAFE_TRAIT_METHODS.contains(&method_name);
    }
    
    false
}

/// Check if a function is a safe trait method implementation.
///
/// This detects trait method implementations that are safe borrows,
/// even if not from derive macros. For example:
/// `<SomeType as PartialEq>::eq`
#[cfg(feature = "advanced-rules")]
fn is_safe_trait_method(func_name: &str, func_signature: &str) -> bool {
    // Check for trait method patterns in the signature
    let trait_patterns = [
    ...
    // Check if the function name ends with a safe trait method
    let is_safe_method = if let Some(last_segment) = func_name.rsplit("::").next() {
        let method_name = last_segment.split('<').next().unwrap_or(last_segment);
        SAFE_TRAIT_METHODS.contains(&method_name)
    } else {
        false
    };
    ...
    // Check if the signature or name indicates a trait implementation
    for pattern in trait_patterns {
        if func_name.contains(pattern) || func_signature.contains(pattern) {
            return true;
        }
    }
    
    // Check for `<Type as Trait>` pattern in the name
    if func_name.contains(" as ") && func_name.contains(">::") {
        return true;
    }
...
/// Check if ADV001 should skip this function to avoid false positives.
///
/// ADV001 (pointer escapes) generates many false positives on:
/// 1. Derive macro generated code (PartialEq, Hash, Debug, etc.)
/// 2. Safe trait method implementations where references are borrows, not escapes
#[cfg(feature = "advanced-rules")]
fn should_skip_adv001(func_name: &str, func_signature: &str) -> bool {
    is_derive_macro_function(func_name) || is_safe_trait_method(func_name, func_signature)
}
...
fn run_advanced_rules(package: &MirPackage) -> Vec<Finding> {
    ...
    for func in &package.functions {
        ...
        for rule in &rules {
            // Skip ADV001 for derive macro functions and safe trait methods
            // to reduce false positives (references passed to these are safe borrows)
            if rule.id() == "ADV001" && should_skip_adv001(&func.name, &func.signature) {
                continue;
            }
