...
fn test_summary_generation_on_interprocedural_examples() {
    ...
    println!("Parsed MIR package with {} functions", package.functions.len());
    ...
    println!("\nCall graph created with {} functions", analysis.call_graph.nodes.len());
    println!("Analysis order: {} functions", analysis.call_graph.analysis_order.len());
    ...
    if let Some(summary) = analysis.get_summary("get_user_input") {
        ...
        assert!(
            !matches!(summary.return_taint, mir_extractor::interprocedural::ReturnTaint::Clean),
...
fn test_summary_propagation() {
    ...
    if has_two_level {
        ...
        println!("  return_taint: {:?}", summary.return_taint);
        println!("  propagation_rules: {} rules", summary.propagation_rules.len());
