...
Static security analyzer for Rust. Compiles source code to access MIR (Mid-level Intermediate Representation) and HIR (High-level Intermediate Representation), which are internal compiler formats that reveal issues invisible to source-level scanners (expanded macros, resolved generics, trait implementations). Integrates with your favorite Code LLM or Code Assistant to filter false positives and suggest fixes.
